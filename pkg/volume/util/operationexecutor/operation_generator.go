...
func (og *operationGenerator) GenerateVolumesAreAttachedFunc(
	...
	for _, volumeAttached := range attachedVolumes {
		if volumeAttached.VolumeSpec == nil {
			klog.ErrorS(nil, "VerifyVolumesAreAttached.GenerateVolumesAreAttachedFunc: nil spec for volume", "volumeName", volumeAttached.VolumeName)
			continue
		}
		volumePlugin, err :=
			og.volumePluginMgr.FindPluginBySpec(volumeAttached.VolumeSpec)
		if err != nil || volumePlugin == nil {
			klog.ErrorS(err, "VolumesAreAttached.FindPluginBySpec failed", "volumeName", volumeAttached.VolumeName, "nodeName", volumeAttached.NodeName)
			continue
		}
	...
	volumesAreAttachedFunc := func() volumetypes.OperationContext {
		...
		for pluginName, volumesSpecs := range volumesPerPlugin {
			...
			if err != nil || attachableVolumePlugin == nil {
				klog.ErrorS(err, "VolumeAreAttached.FindAttachablePluginBySpec failed",
			...
			if newAttacherErr != nil {
				klog.ErrorS(newAttacherErr, "VolumesAreAttached.NewAttacher failed",
					"volumePluginName", pluginName)
				continue
			}
			...
			if areAttachedErr != nil {
				klog.ErrorS(areAttachedErr, "VolumesAreAttached failed",
					"nodeName", nodeName)
				continue
			}
			...
			for spec, check := range attached {
				if !check {
					...
					klog.V(1).InfoS("VerifyVolumesAreAttached determined volume is no longer attached, therefore it was marked as detached",
						"volumeName", volumeSpecMap[spec],
						"volumeSpecName", spec.Name(),
						"nodeName", nodeName)
				}
			}
		}
...
func (og *operationGenerator) GenerateBulkVolumeVerifyFunc(
	...
	bulkVolumeVerifyFunc := func() volumetypes.OperationContext {
		attachableVolumePlugin, err :=
			og.volumePluginMgr.FindAttachablePluginByName(pluginName)
		if err != nil || attachableVolumePlugin == nil {
			klog.ErrorS(err, "BulkVerifyVolume.FindAttachablePluginBySpec failed",
				"volumePluginName", pluginName)
			return volumetypes.NewOperationContext(nil, nil, false)
		}
		...
		if newAttacherErr != nil {
			klog.ErrorS(newAttacherErr, "BulkVerifyVolume.NewAttacher failed",
				"volumePluginName", attachableVolumePlugin.GetPluginName())
			return volumetypes.NewOperationContext(nil, nil, false)
		}
		bulkVolumeVerifier, ok := volumeAttacher.(volume.BulkVolumeVerifier)

		if !ok {
			klog.ErrorS(nil, "BulkVerifyVolume failed to type assert attacher", "bulkVolumeVerifier", bulkVolumeVerifier)
			return volumetypes.NewOperationContext(nil, nil, false)
		}

		attached, bulkAttachErr := bulkVolumeVerifier.BulkVerifyVolumes(pluginNodeVolumes)
		if bulkAttachErr != nil {
			klog.ErrorS(bulkAttachErr, "BulkVerifyVolume.BulkVerifyVolumes failed to check volumes are attached")
			return volumetypes.NewOperationContext(nil, nil, false)
		}
		...
		for nodeName, volumeSpecs := range pluginNodeVolumes {
			for _, volumeSpec := range volumeSpecs {
				...
				if !nodeChecked {
					klog.V(2).InfoS("VerifyVolumesAreAttached.BulkVerifyVolumes failed and leaving volume as attached",
						"nodeName", nodeName,
				...
				if !check {
					klog.V(2).InfoS("VerifyVolumesAreAttached.BulkVerifyVolumes failed",
						"nodeName", nodeName,
	...
	attachVolumeFunc := func() volumetypes.OperationContext {
		...
		if attachErr != nil {
			...
			addErr := actualStateOfWorld.MarkVolumeAsUncertain(
				...
				uncertainNode)
			if addErr != nil {
				klog.ErrorS(addErr, "AttachVolume.MarkVolumeAsUncertain fail to add the volume to actual state",
		...
		klog.InfoS(volumeToAttach.GenerateMsgDetailed("AttachVolume.Attach succeeded", ""))
...
func (og *operationGenerator) GenerateDetachVolumeFunc(
	...
	detachVolumeFunc := func() volumetypes.OperationContext {
		...
		klog.InfoS(volumeToDetach.GenerateMsgDetailed("DetachVolume.Detach succeeded", ""))
...
func (og *operationGenerator) GenerateMountVolumeFunc(
	...
	mountVolumeFunc := func() volumetypes.OperationContext {
		...
		if volumeAttacher != nil {
			...
			klog.InfoS(volumeToMount.GenerateMsgDetailed("MountVolume.WaitForAttach entering", fmt.Sprintf("DevicePath %q", volumeToMount.DevicePath)))
			...
			klog.InfoS(volumeToMount.GenerateMsgDetailed("MountVolume.WaitForAttach succeeded", fmt.Sprintf("DevicePath %q", devicePath)))
		}
		...
		if volumeDeviceMounter != nil && actualStateOfWorld.GetDeviceMountState(volumeToMount.VolumeName) != DeviceGloballyMounted {
			...
			klog.InfoS(volumeToMount.GenerateMsgDetailed("MountVolume.MountDevice succeeded", fmt.Sprintf("device mount path %q", deviceMountPath)))
			...
			if resizeError != nil {
				klog.ErrorS(resizeError, "MountVolume.NodeExpandVolume failed")
		...
		klog.V(verbosity).InfoS(detailedMsg)
...
func (og *operationGenerator) markDeviceErrorState(volumeToMount VolumeToMount, devicePath, deviceMountPath string, mountError error, actualStateOfWorld ActualStateOfWorldMounterUpdater) {
	if volumetypes.IsOperationFinishedError(mountError) &&
		...
		if markDeviceUnmountError != nil {
			klog.ErrorS(markDeviceUnmountError, "MountDevice.MarkDeviceAsUnmounted failed", "volumeName", volumeToMount.VolumeName, "pod", klog.KObj(volumeToMount.Pod))
		}
		return
	}
	...
	if volumetypes.IsUncertainProgressError(mountError) &&
		...
		if markDeviceUncertainError != nil {
			klog.ErrorS(markDeviceUncertainError, "MountDevice.MarkDeviceAsUncertain failed", "volumeName", volumeToMount.VolumeName, "pod", klog.KObj(volumeToMount.Pod))
		}
	}
...
func (og *operationGenerator) markVolumeErrorState(volumeToMount VolumeToMount, markOpts MarkVolumeOpts, mountError error, actualStateOfWorld ActualStateOfWorldMounterUpdater) {
	if volumetypes.IsOperationFinishedError(mountError) &&
		...
		if t != nil {
			klog.ErrorS(t, "MountVolume.MarkVolumeAsUnmounted failed", "volumeName", volumeToMount.VolumeName, "pod", klog.KObj(volumeToMount.Pod))
		}
		return
	}
	...
	if volumetypes.IsUncertainProgressError(mountError) &&
		...
		if t != nil {
			klog.ErrorS(t, "MountVolume.MarkVolumeMountAsUncertain failed", "volumeName", volumeToMount.VolumeName, "pod", klog.KObj(volumeToMount.Pod))
		}
	}
...
func (og *operationGenerator) GenerateUnmountVolumeFunc(
	...
	unmountVolumeFunc := func() volumetypes.OperationContext {
		...
		klog.InfoS("UnmountVolume.TearDown succeeded",
			"volume", volumeToUnmount.VolumeName,
			"OuterVolumeSpecName", volumeToUnmount.OuterVolumeSpecName,
			"podName", volumeToUnmount.PodName,
			"podUID", volumeToUnmount.PodUID,
			"InnerVolumeSpecName", volumeToUnmount.InnerVolumeSpecName,
			"PluginName", volumeToUnmount.PluginName,
			"VolumeGidValue", volumeToUnmount.VolumeGidValue)
		...
		if markVolMountedErr != nil {
			...
			klog.ErrorS(markVolMountedErr, "UnmountVolume.MarkVolumeAsUnmounted failed", "volumeName", volumeToUnmount.VolumeName, "podName", volumeToUnmount.PodName, "podUID", volumeToUnmount.PodUID)
		}
...
func (og *operationGenerator) GenerateUnmountDeviceFunc(
	...
	unmountDeviceFunc := func() volumetypes.OperationContext {
		...
		if err != nil {
			...
			deviceMountPath = deviceToDetach.DeviceMountPath
			_, e2 := deviceToDetach.GenerateMsg(fmt.Sprintf(
		...
		klog.InfoS(deviceToDetach.GenerateMsg("UnmountDevice succeeded", ""))
...
func (og *operationGenerator) GenerateMapVolumeFunc(
	...
	mapVolumeFunc := func() (operationContext volumetypes.OperationContext) {
		...
		if volumeAttacher != nil {
			// Wait for attachable volumes to finish attaching
			klog.InfoS(volumeToMount.GenerateMsgDetailed("MapVolume.WaitForAttach entering", fmt.Sprintf("DevicePath %q", volumeToMount.DevicePath)))
			...
			klog.InfoS(volumeToMount.GenerateMsgDetailed("MapVolume.WaitForAttach succeeded", fmt.Sprintf("DevicePath %q", devicePath)))
		...
		_, resizeError := og.nodeExpandVolume(volumeToMount, actualStateOfWorld, resizeOptions)
		if resizeError != nil {
			klog.ErrorS(resizeError, "MapVolume.NodeExpandVolume failed")
...
func (og *operationGenerator) GenerateUnmapVolumeFunc(
	...
	unmapVolumeFunc := func() volumetypes.OperationContext {
		...
		klog.InfoS("UnmapVolume succeeded",
			"volumeName", volumeToUnmount.VolumeName,
			"outerVolumeSpecName", volumeToUnmount.OuterVolumeSpecName,
			"podName", volumeToUnmount.PodName,
			"podUID", volumeToUnmount.PodUID,
			"innerVolumeSpecName", volumeToUnmount.InnerVolumeSpecName,
			"volumePluginName", volumeToUnmount.PluginName,
			"volumeGidValue", volumeToUnmount.VolumeGidValue)
		...
		if markVolUnmountedErr != nil {
			// On failure, just log and exit
			klog.ErrorS(markVolUnmountedErr, "UnmapVolume.MarkVolumeAsUnmounted failed", "volumeName", volumeToUnmount.VolumeName, "podName", volumeToUnmount.PodName, "podUID", volumeToUnmount.PodUID)
		}
...
func (og *operationGenerator) GenerateUnmapDeviceFunc(
	...
	unmapDeviceFunc := func() volumetypes.OperationContext {
		...
		klog.InfoS(deviceToDetach.GenerateMsgDetailed("UnmapDevice succeeded", ""))
	...
	verifyControllerAttachedVolumeFunc := func() volumetypes.OperationContext {
		...
		for _, attachedVolume := range node.Status.VolumesAttached {
			if attachedVolume.Name == volumeToMount.VolumeName {
				...
				klog.InfoS(volumeToMount.GenerateMsgDetailed("Controller attach succeeded", fmt.Sprintf("device path: %q", attachedVolume.DevicePath)))
				if addVolumeNodeErr != nil {
					// On failure, return error. Caller will log and retry.
...
func (og *operationGenerator) verifyVolumeIsSafeToDetach(
	...
	if fetchErr != nil {
		if errors.IsNotFound(fetchErr) {
			klog.InfoS(volumeToDetach.GenerateMsgDetailed("Node not found on API server. DetachVolume will skip safe to detach check", ""))
			return nil
		}
	...
	klog.InfoS(volumeToDetach.GenerateMsgDetailed("Verified volume is safe to detach", ""))
	return nil
}
...
func (og *operationGenerator) GenerateExpandVolumeFunc(
	...
	expandVolumeFunc := func() volumetypes.OperationContext {
		...
		if pvSize.Cmp(newSize) < 0 {
			...
			klog.InfoS("ExpandVolume succeeded", "pvcQualifiedName", util.GetPersistentVolumeClaimQualifiedName(pvc))
			...
			klog.InfoS("ExpandVolume.UpdatePV succeeded", "pvcQualifiedName", util.GetPersistentVolumeClaimQualifiedName(pvc))
		}
		...
		if !volumePlugin.RequiresFSResize() || !fsVolume {
			klog.V(4).InfoS("Controller resizing done", "pvcQualifiedName", util.GetPersistentVolumeClaimQualifiedName(pvc))
...
func (og *operationGenerator) nodeExpandVolume(
	...
	if !utilfeature.DefaultFeatureGate.Enabled(features.ExpandPersistentVolumes) {
		klog.V(4).InfoS("Resizing is not enabled", "volumeName", volumeToMount.VolumeName)
		return true, nil
	}

	if volumeToMount.VolumeSpec != nil &&
		volumeToMount.VolumeSpec.InlineVolumeSpecForCSIMigration {
		klog.V(4).InfoS("Volume is a migrated inline volume and is not resizable", "volumeName", volumeToMount.VolumeName)
		return true, nil
	}
	...
	if expandableVolumePlugin != nil &&
		...
		if pvcStatusCap.Cmp(pvSpecCap) < 0 {
			...
			klog.V(4).InfoS(volumeToMount.GenerateMsgDetailed("MountVolume.NodeExpandVolume entering", fmt.Sprintf("DevicePath %q", volumeToMount.DevicePath)))
			...
			if resizeErr != nil {
				...
				if volumetypes.IsFailedPreconditionError(resizeErr) {
					...
					klog.ErrorS(resizeErr, "MountVolume.NodeExapndVolume failed", "volumeName", volumeToMount.VolumeName, "pod", klog.KObj(volumeToMount.Pod))
					return true, nil
				}
...
func isDeviceOpened(deviceToDetach AttachedVolume, hostUtil hostutil.HostUtils) (bool, error) {
	...
	if !isDevicePath && devicePathErr == nil ||
		...
		//TODO: refer to #36092
		klog.V(3).InfoS("The path isn't device path or doesn't exist. Skip checking device", "devicePath", deviceToDetach.DevicePath)
		deviceOpened = false
